function customers(page = 1, search = $('#search').val()) {
    loading();
    $.ajax({
        type: 'POST',
        ...
        success: function(e) {
            ...
            if (parseInt(e.results) > 0) {
                $.each(e.data, function(i, d) {
                    table += '<tr class="customers_list" onclick="show_customer(\'' + d.email + '\')"><td>' + (e.start++) + '.</td><td>' + d.email + '</td><td>' + d.firstname + ' ' + d.lastname + '</td><td>' + d.orders + '</td><td>$' + d.spent + '</td></tr>';
                });
            } else {
